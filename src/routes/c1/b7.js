...
router.get("/c1/b7/procOrdenRetiro", isLoggedIn, async (req, res) => {
	var sql, rs, msg;
	try {
		var nroRec = req.body.nroRec;
		sql = `select COUNT(*) cont from retiroauto0km where retirado=1 and codRA0KM=${nroRec}`;
		rs = await pool.query(sql);
		if (rs[0].cont === 1) {
			msg =
				"El automóvil correspondiente a la órden de retiro ingresada ya ha sido retirado";
		} else {
			sql = `select COUNT(*) cont from retiroauto0km where retirado=0 and CODRA0KM=${nroRec}`;
			rs = await pool.query(sql);
			if (rs[0].cont === 0) {
				msg = "La órden de retiro ingresada es inválida o inexistente.";
			} else {
				sql = `UPDATE retiroauto0km SET retirado=1 where codRA0KM=${nroRec}`;
